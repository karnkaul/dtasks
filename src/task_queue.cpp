...
task_queue::task_queue(std::uint8_t worker_count) {
	...
	m_queue.active(true);
	m_next_task.store(0);
	for (std::uint8_t i = 0; i < worker_count; ++i) {
		m_workers.push_back(worker(m_status, m_queue));
	}
}
